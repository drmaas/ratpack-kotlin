...
class KChain(val delegate: Chain) {
  ...
  inline fun chain(crossinline cb: KChain.(c: KChain) -> Unit): Handler = delegate.chain { val c = KChain(it); c.cb(c) }
  fun chain(action: Class<out Action<in Chain>>): Handler = delegate.chain(action)

  inline fun files(crossinline cb: FileHandlerSpec.(f: FileHandlerSpec) -> Unit): KChain = KChain(delegate.files { it.cb(it) })
  fun files(): KChain = KChain(delegate.files())
  fun files(action: Action<in FileHandlerSpec>): KChain = KChain(delegate.files(action))

  inline fun fileSystem(path: String = "", crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.fileSystem(path) { val c = KChain(it); c.cb(c) })
  ...
  inline fun prefix(path: String = "", crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.prefix(path) { val c = KChain(it); c.cb(c) })
  ...
  inline fun all(crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.all { val c = KContext(it); c.cb(c) })
  ...
  inline fun path(path: String = "", crossinline cb: KContext.(ctx: KContext) -> Unit): KChain = KChain(delegate.path(path) { val ctx = KContext(it); ctx.cb(ctx) })
  ...
  inline fun host(hostName: String, crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.host(hostName) { val c = KChain(it); c.cb(c) })
  ...
  inline fun insert(crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.insert { val c = KChain(it); c.cb(c) })
  ...
  inline fun get(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.get(path) { val c = KContext(it); c.cb(c) })
  ...
  inline fun put(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.put(path) { val c = KContext(it); c.cb(c) })
  ...
  inline fun post(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.post(path) { val c = KContext(it); c.cb(c) })
  ...
  inline fun delete(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.delete(path) { val c = KContext(it); c.cb(c) })
  ...
  inline fun options(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.options(path) { val c = KContext(it); c.cb(c) })
  ...
  inline fun patch(path: String = "", crossinline cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.patch(path) { val c = KContext(it); c.cb(c) })
  ...
  fun onlyIf(test: Predicate<in Context>, cb: KContext.(c: KContext) -> Unit): KChain = KChain(delegate.onlyIf(test, KHandlers.from(cb)))
  ...
  inline fun `when`(test: Predicate<in Context>, crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.`when`({ ctx: Context -> test.apply(ctx) }) { val c = KChain(it); c.cb(c) })
  inline fun `when`(test: Boolean, crossinline cb: KChain.(c: KChain) -> Unit): KChain = KChain(delegate.`when`(test) { val c = KChain(it); c.cb(c) })
